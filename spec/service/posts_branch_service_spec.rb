...
require './app/services/posts_branch_service.rb'

describe PostsBranchService do
  ...
  context '#call' do
    ...
    it 'returns posts filtered by a branch' do
      ...
      expect(PostsBranchService.new({branch: 'hobby'}).call).to(
    ...
    it 'returns posts filtered by a branch and a search input' do
      ...
      category
      included_post = [] << post
      expect(PostsBranchService.new({branch: 'hobby', search: 'fun'}).call).to(
        eq included_post
      )
    end
    ...
    it 'returns posts filtered by a category name' do
      ...
      category
      included_post = [] << post
      expect(PostsBranchService.new({branch: 'hobby', category: 'arts'}).call).to(
        eq included_post
      )
    end
    ...
    it 'returns posts filtered by a category name and a search input' do
      ...
      category
      included_post = [] << post
      expect(PostsBranchService.new({name: 'arts', 
                                     search: 'fun', 
